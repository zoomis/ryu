...
class RestStatsApi(app_manager.RyuApp):
	...
	if not haddr_to_str(src) in self.device:
	...
	if _eth_type == 0x0800:
		...
		self.device[haddr_to_str(src)]['ipv4'] = self.ip_to_str(src_ip)
		LOG.info("IPv4 update %s for %s", self.ip_to_str(src_ip),
			 haddr_to_str(src) )
		self.device.setdefault(haddr_to_str(dst), {})
		self.device[haddr_to_str(dst)]['ipv4'] = self.ip_to_str(dst_ip)
		LOG.info("IPv4 update %s for %s", self.ip_to_str(dst_ip),
			 haddr_to_str(dst) )

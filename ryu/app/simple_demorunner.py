...
class SimpleIsolation(app_manager.RyuApp):
    ...
    def _flood_to_nw_id(self, msg, src, dst, nw_id):
        self.demo_app().handle_packet_in(msg, None)
...
#            self._flood_to_nw_id(msg, src, dst, dst_nw_id)
    ...
    def _is_demo_app_changed(self):
        ...
        print "last mode", self._last_demo_change, stat.st_mtime
    ...
    def demo_app(self):
        ...
        if not self._demo_app or self._is_demo_app_changed():
          print "reeeeeeeeeeeeeloadding"
          self._demo_app = demo_app.DemoApp()
